// ============================================================
//  COSC 182 – Java Midterm Quiz  |  quiz.js
//  Quiz logic: shuffle, render, score, review
// ============================================================

// ── State ────────────────────────────────────────────────────
const state = {
  questions:    [],   // 20 selected questions for this session
  current:      0,    // current question index
  answers:      [],   // { type, selected|grade, correct } per question
  startTime:    null,
  timerInterval:null
};
...
function buildQuiz() {
  ...
  // Interleave: alternate MC and coding or just concat then shuffle within groups
  // For clarity: first 10 are MC, next 10 are coding — display labels differentiate them
...
function renderQuestion() {
  ...
  // Re-attach dynamic events after re-render
...
function buildCardHTML(q, idx, isMC) {
  ...
  let body = '';
  if (isMC) body = buildMCHTML(q, saved);
  else      body = buildCodeHTML(q, saved);
...
function buildMCHTML(q, saved) {
  ...
  const explainShow = submitted ? 'show' : '';
  const correctText = submitted
    ? (saved.correct ? '✅ Correct!' : `❌ The correct answer is <strong>${letters[q.answer]}: ${q.options[q.answer]}</strong>`)
    : '';

  return `
    <ul class="options-list" id="mc-options">
      ${optionsHTML}
    </ul>
    <div class="explanation-box ${explainShow}" id="explanation">
...
function attachMCEvents(idx) {
  ...
  const saved  = state.answers[idx];
  if (saved !== null) return; // already answered
...
function buildCodeHTML(q, saved) {
  ...
  return `
    <div class="coding-area">
      ...
      <div class="answer-reveal ${revealClass}" id="answer-reveal">
        ...
        ${!submitted
          ? `<div class="self-grade">
               ...
               <button class="btn btn-danger btn-sm"  id="btn-grade-wrong">❌ I need more practice</button>
             </div>`
          : ''
        }
      </div>
...
function attachCodeEvents(idx) {
  ...
  if (saved !== null) return; // already graded
...
function finishQuiz() {
  ...
  document.getElementById('results-summary').textContent =
    ...
    pct >= 60 ? '📖 Decent start — review the topics you missed.' :
                '💪 Keep studying — revisit chapters 17–19 and Assign3.';
